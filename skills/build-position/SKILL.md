...
5. Проверь OUTPUT:
   ...
   - верхнеуровневые `## ` секции ограничены только списком Ф5b;
   - присутствует `## Схема сторон`;
   - в секции `## Схема сторон` есть блок ````mermaid```.
6. Если проверка не проходит, перезапусти Opus-subagent один раз с напоминанием о контракте 3.1a, разрешённых секциях и обязательном `READY_FOR_DOCX: analytical`. Если второй ответ невалиден — остановись и покажи блокер.
