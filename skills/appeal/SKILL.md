...
name: appeal
description: >
  Апелляционная жалоба по single-document ветке Ф5d. Sonnet-main оркестрирует,
  Opus-subagent анализирует основания обжалования по контракту 3.1a,
  Sonnet-subagent оформляет .docx через arbitrum-docx по контракту 3.2.
---

# appeal — Апелляционная жалоба

Скилл следует контрактам из [shared/subagent-dispatch.md](/Users/strigov/Documents/Claude/Projects/Suzerain/plugins/vassal-litigator/shared/subagent-dispatch.md). Итог ветки Ф5d для `appeal`: один `.docx` в корне дела.

## Предусловия

- Существует `.vassal/case.yaml`.
- Существует `.vassal/index.yaml`.
- Загружено решение, которое обжалуется, либо его зеркало.
...
- `docx_path` = `<case_root>/{{today}} appeal.docx`
...
4. Покажи Сюзерену preview:
   - какие документы пойдут в контекст;
...
2. Собери prompt Opus-subagent по контрактам 3.0 и 3.1a:
   - `ROLE`: `Ты юрист-аналитик в скилле appeal.`
   ...
   - `TASK`:
     - проанализировать основания для апелляционного обжалования по АПК/ГПК;
     - ранжировать доводы и собрать структуру жалобы;
...
- Не превращать апелляцию в multi-document ветку.
...
- не подменяй анализ апелляционных оснований ручной генерацией main-Sonnet.
